...
// će se graf nalaziti, playerNum koji označava lijevog (1) ili desnog (2) igrača
function drawBarChart(data, allowedKeys, element, playerNum, abb) {
  // Brisanje elementa pri novom crtanju
  if (d3.select(`#barchart${playerNum}`)) {
    d3.select(`#player${playerNum}data`).selectAll("*").remove();
  }
  ...
  var y = d3.scale
    .ordinal()
    .rangeRoundBands([heightBar, 0], 0.1)
    .domain(
      data.map(function (d) {
        return d.axis;
      })
  ...
  svgBar
    ...
    .selectAll(".tick text") // Odabir teksta unutar oznaka na osi
    .text(function (d) {
      return abb[d]; // Koristi skraćenicu umjesto punog naziva atributa
    });

  // Dodavanje g elemenata za podatke
  var bars = svgBar.selectAll(".bar").data(data).enter().append("g");
